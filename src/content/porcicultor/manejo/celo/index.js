...
function LineasGeneticasListado() {
    ...
    const getListado = useCallback(async (reqObj) => {
      setLoading(true)
        try {
          const response = await certifyAxios.post(celoQueryAPI, reqObj);
    ...
    // edit
    const navigateToDetalle = (id, nombre, disableStatusCerda = false) => {
      navigate('/sp/porcicultor/manejo/celo/lote-detalle', {state:{loteId: id, loteNombre:nombre, disableStatusCerda }});
    };
