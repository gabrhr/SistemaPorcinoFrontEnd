...
import { corralDeleteAPI, corralQueryAPI, corralRegisterAPI, corralUpdateAPI } from 'src/utils/apiUrls';
...
const tituloPagina = "Corrales"
const itemSingular = "Corral"

function CorralesListado() {
    ...
    const getListado = useCallback(async (reqObj) => {
        setLoading(true)
        try {
          const response = await certifyAxios.post(corralQueryAPI, reqObj);
    ...
    const addItem = async (reqObj, afterAction) => {
      try {
        const response = await certifyAxios.post(corralRegisterAPI, reqObj);
    ...
    const editItemById = async (reqObj, afterAction) => {
      try {
        const response = await certifyAxios.post(corralUpdateAPI, reqObj);
        if(response.data?.resultCode === resultCodeOk){
          getListado(defaultObj)
    ...
    const deleteItemById = async (id, afterDelete) => {
      try {
        ...
        const response = await certifyAxios.post(corralDeleteAPI, reqObj);
        if(response.data?.resultCode === resultCodeOk){
          getListado(defaultObj)
...
export default CorralesListado;