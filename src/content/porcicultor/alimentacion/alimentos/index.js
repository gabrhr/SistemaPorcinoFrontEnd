...
import { alimentoDeleteAPI, alimentoQueryAPI } from 'src/utils/apiUrls';
...
function AlimentosListado() {
    ...
    const getListado = useCallback(async (reqObj) => {
      setLoading(true)
        try {
          const response = await certifyAxios.post(alimentoQueryAPI, reqObj);
    ...
    const navigateToDetalle = (id) => {
      navigate('/sp/porcicultor/alimentacion/alimentos/detalle', {state:{alimentoId: id}});
    };
...
export default AlimentosListado;